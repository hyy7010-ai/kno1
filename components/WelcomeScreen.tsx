...
import { ArrowRight, Zap, Brain, Sparkles, Layers, Mic, ShieldCheck, FlaskConical, Database, Inbox, BrainCircuit, MousePointer2, Plus, LayoutGrid, Shield, Download, HardDrive, FileOutput, MessageSquare } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-white z-[100] animate-fade-in selection:bg-black selection:text-white font-sans overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center space-y-6 w-full">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full">
                ...
                <div className="flex gap-2">
                    <button 
                        ...
                        className="group relative inline-flex items-center justify-center px-6 py-5 bg-white text-black border-2 border-black rounded-full font-bold text-sm uppercase tracking-widest hover:bg-gray-50 active:scale-95 transition-all"
                    >
                        <span className="relative z-10 flex items-center">
                            <Download className="w-4 h-4 mr-2" /> {t('MAC', getSystemLanguage())}
                        </span>
                    </button>
                    <button 
                        onClick={onShowPricing}
                    ...
                    >
                        <span className="relative z-10 flex items-center">
                            <Download className="w-4 h-4 mr-2" /> {t('WINDOWS', getSystemLanguage())}
                        </span>
                    </button>
                </div>
            </div>
            ...
            <button 
                onClick={onShowPricing}
                className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] hover:text-blue-600 transition-colors py-2 mt-4"
            >
                {t('VIEW PRICING & TIERS', getSystemLanguage())}
            </button>
        </div>
      </div>
      </div>
